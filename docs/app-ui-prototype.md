...
首版先做“人机练习桌”的完整 UI 和交互原型：用户坐在底部，其余 5 家由 AI 占位。当前已在后端接口测试通过后开放最小在线房间入口，用于创建/加入房间、准备、开局和服务端快照同步；好友组局、语音等功能后续扩展。
...
- `playing_cards`：用于快速渲染标准扑克牌牌面、背面和 Joker。它提供 `PlayingCardView`、`standardFiftyFourCardDeck()`、`FlatCardFan`，适合先把“我的手牌”和中央出牌区做出来。天津基础规则使用 1 副牌；若其他扩展规则使用多副牌，业务模型里仍要给每张牌加唯一 `cardId`，不能只用牌面表示一张牌。
...
`card_game` 特别说明：这个包很适合纸牌拖拽类游戏，支持标准牌、拖放校验、卡牌移动动画、`CardRow` / `CardColumn` / `CardDeck` 等布局。但砸六家核心交互更像“选多张手牌 -> 识别牌型 -> 点击出牌/过牌”；如果后续扩展到多副牌，需要给业务牌对象包装唯一 ID，并确认它的拖拽模型不会干扰移动端单手出牌体验。因此首版推荐只借鉴它的思路，不直接绑定整套牌桌。
...
3. 用自定义 `CardInstance` 模型包住牌面，保留唯一 ID，方便后续扩展到多副牌。
...
- 中央出牌区能显示上家牌型和当前轮到谁
- 出牌、过牌、提示按钮状态正确
- 手牌牌面由 `playing_cards` 或等价组件渲染，Joker 能正确显示，牌对象保留唯一 ID
- 有玩家出完后能显示名次
...
1. 在线房间首版是否只保留创建/加入/准备/开局/出牌同步，不加入聊天和邀请分享？
2. 是否启用进贡、还贡、抗贡、接风？
3. 基础混儿以外的复杂混牌桌规是否需要首版支持？
4. 单局结算是只显示胜负，还是也记录个人名次积分？
5. App 首版强制竖屏，还是同时适配横屏？
6. 扑克牌 UI 是否接受先用 `playing_cards` 默认牌面，后续再换自定义美术？
