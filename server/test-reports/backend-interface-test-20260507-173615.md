...
- 测试时间：2026-05-07 17:36:15
- 测试地址：http://127.0.0.1:59256
- 测试模式：本地自动拉起服务（内存状态）
- 测试结论：未发现后端接口阻断性问题
...
| 编号 | 测试对象 | 场景 | 结果 | 耗时 | 说明 |
| --- | --- | --- | --- | ---: | --- |
| H01 | GET /health | 健康检查 | 通过 | 3ms | 服务可用，Redis=false |
| H02 | GET /api/v1/rooms/by-code/:roomCode | 未知房间号返回结构化 404 | 通过 | 1ms | 符合预期 |
| H03 | POST /api/v1/rooms | 创建房间拒绝空昵称 | 通过 | 6ms | 符合预期 |
| H04 | POST /api/v1/rooms | 创建房间拒绝非法座位 | 通过 | 1ms | 符合预期 |
| H05 | POST /api/v1/rooms | 创建房间成功 | 通过 | 2ms | 房间号 508675 |
...
| H10 | POST /api/v1/rooms/by-code/:roomCode/join | 加入房间成功 | 通过 | 1ms | 符合预期 |
| H11 | GET /api/v1/rooms/:roomId/snapshot | 快照接口缺 token 返回 401 | 通过 | 0ms | 符合预期 |
| H12 | GET /api/v1/rooms/:roomId/snapshot | 快照接口错误 token 返回 401 | 通过 | 1ms | 符合预期 |
...
| H17 | POST /api/v1/rooms/by-code/:roomCode/join | 加入满房时返回 ROOM_FULL | 通过 | 4ms | 符合预期 |
...
| 编号 | 测试对象 | 场景 | 结果 | 耗时 | 说明 |
| --- | --- | --- | --- | ---: | --- |
| W01 | POST /api/v1/rooms + join | 创建联机测试房间 | 通过 | 1ms | 房间号 099824 |
| W02 | WS /ws/v1/tables/:roomId | 错误 token 被拒绝连接 | 通过 | 2ms | 符合预期 |
| W03 | WS table_snapshot | 连接后收到初始快照 | 通过 | 3ms | 符合预期 |
| W04 | WS ping | ping 返回 pong | 通过 | 0ms | 符合预期 |
| W05 | WS play_cards | 未开局时出牌被拒绝 | 通过 | 0ms | 符合预期 |
| W06 | WS start_game | 非房主不能开局 | 通过 | 0ms | 符合预期 |
| W07 | WS start_game | 房主在玩家未准备时不能开局 | 通过 | 0ms | 符合预期 |
| W08 | WS ready | ready 状态广播 | 通过 | 0ms | 符合预期 |
| W09 | WS start_game | 房主开局成功，AI 补齐且仅返回自己的手牌 | 通过 | 6ms | 符合预期 |
| W11 | WS sync_state | sync_state 返回最新快照 | 通过 | 0ms | 符合预期 |
...
| W16 | WS reconnect | 断线后用同 token 重连可恢复快照 | 通过 | 153ms | 符合预期 |
