...
- 测试时间：2026-05-08 01:53:22 Asia/Shanghai
- 测试地址：http://127.0.0.1:64823
- 测试模式：本地自动拉起服务（内存状态）
- 测试结论：未发现后端接口阻断性问题
...
| H02 | GET /api/v1/rooms/by-code/:roomCode | 未知房间号返回结构化 404 | 通过 | 2ms | 符合预期 |
| H03 | POST /api/v1/rooms | 创建房间拒绝空昵称 | 通过 | 3ms | 符合预期 |
| H04 | POST /api/v1/rooms | 创建房间拒绝非法座位 | 通过 | 1ms | 符合预期 |
| H05 | POST /api/v1/rooms | 创建房间成功 | 通过 | 2ms | 房间号 665258 |
...
| H13 | GET /api/v1/rooms/:roomId/snapshot | 快照接口成功且不泄露他人手牌 | 通过 | 1ms | 符合预期 |
| H14 | GET /api/v1/players/me/stats | 个人战绩接口成功 | 通过 | 0ms | 符合预期 |
...
| 编号 | 测试对象 | 场景 | 结果 | 耗时 | 说明 |
| --- | --- | --- | --- | ---: | --- |
| W01 | POST /api/v1/rooms + join | 创建联机测试房间 | 通过 | 1ms | 房间号 920509 |
...
| W05 | WS play_cards | 未开局时出牌被拒绝 | 通过 | 1ms | 符合预期 |
...
| W14 | WS pass | 当前玩家领出时不能过牌 | 通过 | 0ms | 当前桌面已有牌，此局面不是领出，跳过领出过牌校验 |
